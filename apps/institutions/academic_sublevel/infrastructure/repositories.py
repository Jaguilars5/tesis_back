...
class AcademicSublevelRepository(BaseRepository, AcademicSublevelRepositoryInterface):
    ...
    def get_cascade_counts(cls, instance_id: int) -> dict[str, int]:
        grade_ids = AcademicGrade.objects.filter(academic_sublevel_id=instance_id, is_active=True).values_list("id", flat=True)
        counts = {}
        ...
        scale_count = QualitativeScaleSublevel.objects.filter(sublevel_id=instance_id, is_active=True).count()
        if scale_count:
    ...
    def deactivate_cascade(cls, instance_id: int) -> int:
        grade_ids = list(AcademicGrade.objects.filter(academic_sublevel_id=instance_id, is_active=True).values_list("id", flat=True))
        ...
        total += QualitativeScaleSublevel.objects.filter(sublevel_id=instance_id, is_active=True).update(is_active=False)
