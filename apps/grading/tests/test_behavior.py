...
class BehaviorEvaluationModelTest(TestCase):
    ...
    def setUp(self):
        ...
        self.enrollment = Enrollment.objects.create(
            student=self.student, section=section,
...
class BehaviorEvaluationServiceTest(TestCase):
    ...
    def setUp(self):
        ...
        subj_config = SubjectAcademicConfig.objects.create(
            subject=self.subject, academic_grade=academic_grade,
            weekly_hours=5, pedagogical_order=1,
        )
        offering = SubjectOffering.objects.create(
            school_year=school_year, section=section,
    ...
    def test_calculate_with_moderate_incidents_returns_ac(self):
        for i in range(3):
            ConductIncident.objects.create(
                enrollment=self.enrollment,
                reported_by_user=self.user,
                academic_period=self.period,
                incident_date=date(2025, 1, 10 + i),
                category="disciplina",
                severity=1,
            )
    ...
    def test_calculate_with_severe_incidents_returns_na(self):
        for i in range(3):
            ConductIncident.objects.create(
                enrollment=self.enrollment,
                reported_by_user=self.user,
    ...
    def test_override_evaluation(self):
        evaluation = BehaviorEvaluation.objects.create(
            enrollment=self.enrollment,
            academic_period=self.period,
            calculated_scale=self.scale_na,
        )

        overridden = BehaviorEvaluationService.override_evaluation(
            evaluation, self.scale_se, reason="Mejoria comprobada",
        )

        self.assertEqual(overridden.final_scale.code, "SE")
        self.assertEqual(overridden.override_reason, "Mejoria comprobada")
        self.assertEqual(overridden.calculated_scale.code, "NA")
