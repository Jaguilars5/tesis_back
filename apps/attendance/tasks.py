...
class AttendanceSyncHandler(BaseSyncHandler):
    ...
    def _apply(cls, record_uuid, payload):
        """Upsert por clave natural (enrollment, tss, fecha).

        Es idempotente y tolerante a divergencias de UUID entre el cliente
        offline y el servidor, ya que la unicidad real es la clave natural.
        """
        payload = payload or {}
