...
from apps.analytics.models import AlertType, EarlyAlert, RiskFactor, StudentRiskFactor, StudentRiskScore
...
from apps.grading.models import (
    ActivityType, EvaluationBlock, EvaluativeActivity,
    GradeChangeHistory, LearningReport, PeriodGradeSummary,
    QualitativeScale, RecoveryProcess, RecoveryProcessType,
    RecoverySession, StudentNote,
)
...
class FunctionalWorkflowTests(TestCase):
    """Pruebas de flujos funcionales completos del sistema."""
    ...
    def setUp(self):
        ...
        self.grade = AcademicGrade.objects.create(
            name="7mo", sequence_order=7, academic_sublevel=self.sublevel,
        )
        ...
        self.status_active = EnrollmentStatus.objects.get_or_create(code="ACT", defaults={"name": "Activa"})[0]
        self.enrollment = Enrollment.objects.create(
            student=self.student, section=self.section,
            school_year=self.school_year, enrollment_status=self.status_active,
        )
        ...
        from apps.grading.models import EvaluationBlock, BlockComponent, ComponentIndicator
        ...
        self.comp_indicator = ComponentIndicator.objects.create(
            block_component=self.block_comp, name="Indicador 1",
            internal_weight=Decimal("100.00"),
        )
        ...
        self.eval_activity = EvaluativeActivity.objects.create(
            component_indicator=self.comp_indicator, teacher_subject_section=self.tss,
            title="Examen parcial", activity_type=self.activity_type,
            max_score=Decimal("10.00"), due_date=date(2025, 10, 15),
        )
        self.recovery_type = RecoveryProcessType.objects.get_or_create(
            code="MEJORA_DIRECTA", defaults={"name": "Mejora Directa"},
        )[0]
    ...
    def test_enrollment_to_graduation_workflow(self):
        ...
        summary = PeriodGradeSummary.objects.create(
            ...
            requires_recovery=True, qualitative_scale=self.scale_c,
        )
        recovery = RecoveryProcess.objects.create(
            period_grade_summary=summary, subject_offering=self.offering,
            managed_by_user=self.teacher, process_type=self.recovery_type,
            initial_grade=Decimal("6.00"), start_date=date(2025, 12, 1),
        )
        session = RecoverySession.objects.create(
            recovery_process=recovery, session_date=date(2025, 12, 5),
            duration_minutes=60, topics_covered="Fracciones y decimales",
            student_present=True,
        )
        report = LearningReport.objects.create(
            enrollment=self.enrollment, academic_period=self.period,
            formative_avg=Decimal("6.50"), summative_avg=Decimal("5.50"),
            final_avg=Decimal("7.50"), attendance_rate=Decimal("85.00"),
            general_observations="Mejoró después del refuerzo.", is_final=True,
        )
        self.assertIsNotNone(report.created_at)
        self.assertIsNotNone(note.created_at)
        self.assertTrue(summary.requires_recovery)
        self.assertEqual(session.duration_minutes, 60)
        self.assertEqual(recovery.initial_grade, Decimal("6.00"))
    ...
    def test_behavior_evaluation_workflow(self):
        severity = Severity.objects.create(code="LEVE", name="Falta leve", numeric_level=1)
        ...
        incident = ConductIncident.objects.create(
            enrollment=self.enrollment, reported_by_user=self.teacher,
        ...
        self.assertEqual(incident.severity.numeric_level, 1)
