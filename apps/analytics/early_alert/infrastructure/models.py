"""
Modelos Django para alertas tempranas.

Mantiene la tabla original 'analytics_earlyalert' mediante db_table explícito
...
class EarlyAlert(TimeStampedModel, SyncableModel):
    """
    Alerta temprana generada para un estudiante.

    Mantiene db_table explícito para preservar datos existentes
durante la migración de app_label de 'analytics' a 'early_alert'.
    """
    ...
    attended_at = models.DateTimeField(
        null=True, blank=True, verbose_name="Fecha de atención"
    )
    ...
    class Meta:
        app_label = "early_alert"
        db_table = "analytics_earlyalert"  # Preservar tabla existente
