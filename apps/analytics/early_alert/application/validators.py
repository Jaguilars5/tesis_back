...
def validate_urgency_level(urgency_level: Optional[str]) -> Dict[str, str]:
    ...
    if urgency_level and urgency_level not in dict(AlertModel._meta.get_field("urgency_level").choices):
...
def validate_alert_type(alert_type: Optional[str]) -> Dict[str, str]:
    ...
    if alert_type and alert_type not in dict(AlertModel._meta.get_field("alert_type").choices):
